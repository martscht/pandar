...
* Allein in den letzten 30 Tagen `r format(sum(cranlogs::cran_downloads("R", when="last-month")$count), scientific = F)` mal heruntergeladen
...
![](rstudio.png)
`r blogdown::shortcode("intext_anchor", "Konsole_Syntax")`

RStudio besteht aus vier Panels. Zunächst sind nur drei sichtbar - durch **Strg+Shift+n** (OS X: **Cmd+Shift+n**) oder über den `r blogdown::shortcode("inline_image", "new_script")` Button öffnen Sie eine neue Skriptdatei und das vierte Panel erscheint. `r blogdown::shortcode("intext_anchor", "Eingabe")`
R ist syntaxbasiert - genau genommen ist R eigentlich eine Programmiersprache und kein Auswertungsprogramm - und im neu erschienenen Fenster können Sie diese Syntax schreiben. Das pure schreiben bewirkt zunächst nichts. `r blogdown::shortcode("intext_anchor", "Ergebnisse")`
Damit etwas passiert, muss die Syntax mit **Strg+Return** (OS X: **cmd+Return**) oder mit dem `r blogdown::shortcode("inline_image", "run")` Button ausgeführt werden. Wenn Sie z.B. `3 + 4` in die Syntax schreiben und so ausführen, erscheint in der Konsole:
...
`r blogdown::shortcode("intext_anchor", "Logik")`
...
`r blogdown::shortcode("intext_anchor", "Vektoren")`
