...
```{r, echo = F, purl = F, message=F, warning = F, fig.height=6, fig.width=6}
library(plot3D)
...
```{r, echo = F, purl = F, message=F, warning = F, fig.height=6, fig.width=6, fig.height=6, fig.width=6}
library(plot3D)
# Übersichtlicher: Vorbereitung
x <- c(osf$pas_post)
y <- c(osf$bsi_post)
...
```{r, echo = T, message=F, warning = F, fig.height=6, fig.width=6}
library(plot3D)
# Übersichtlicher: Vorbereitung
x <- c(osf$pas_post)
y <- c(osf$bsi_post)
...
$X$ und $Z$ sind zu `r round(cor(X, Z), 3)` korreliert. Wir zentrieren die beiden Variablen und schauen uns jeweils die Korrelationen untereinander und mit der Interaktion an:
...
cor(X_c, X_c*Z_c)
```
Die Korrelation zwischen $X$ und {{< math >}}$X*Z${{< /math >}} liegt bei `r round(cor(X, X*Z), 3)`, während sie bei den zentrierten Varianten $X_c$ und $X_c*Z_c$ bei `r round(cor(X_c, X_c*Z_c), 3)` liegt. Damit resultiert die erste Variante in Multikollinearitätsproblemen in einem Regressionsmodell, während die zweite dies nicht tut.
