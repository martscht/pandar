...
In der letzten Sitzung haben wir schon einige Befehle für das Screening eines Datensatzes kennen gelernt. Dabei zeigt `names()` alle Variablennamen an, während `dim()` uns Zeilen und Spalten ausgibt. Der Datensatz hat also `r nrow(fb25)` Beobachtungen auf `r ncol(fb25)` Variablen.
...
table(fb25$fach) |> sum()
```
... sieht man, dass nur `r table(fb25$fach) |> sum()` von allen `r nrow(fb25)` Beobachtungen darin Eingang gefunden haben. Das liegt daran, dass der `table`-Befehl ohne zusätzliche Argumente alle `NA`s einfach ignoriert. Um zu sehen, wieviele fehlende Werte wir insgesamt auf dieser Variable haben, müssen wir das Argument `useNA` spezifizieren. `"ifany"` bedeutet, dass fehlende Werte nur mitgezählt werden, falls es welche gibt.
...
Es treten die Werte `r min(fb25$sicher,na.rm=TRUE)` bis  `r max(fb25$sicher,na.rm=TRUE)` empirisch auf, außerdem gibt es  `r sum(is.na(fb25$sicher))` fehlende Werte (dargestellt als `NA`):
...
Ohne Argument für die Behandlung der fehlenden Werte wird `NA` auch als Ergebnis ausgegeben. Mit passendem Argument erhalten wir ein numerisches Ergebnis: Der Median für die Variable `sicher` beträgt also `r median(fb25$sicher, na.rm = TRUE)`.
...
Der Interquartilsbereich liegt also zwischen `r quantile(fb25$sicher, .25, na.rm=TRUE)` und `r quantile(fb25$sicher, .75, na.rm=TRUE)`.
...
Dabei ist in der Ausgabe besonders die Überschrift verwirrend (75%), die hier nichts mit der Bedeutung des Wertes zu tun hat. Der IQA der Variable `sicher` beträgt `r quantile(fb25$sicher, .75, na.rm=TRUE) - quantile(fb25$sicher, .25, na.rm=TRUE)`. Für die Berechnung des IQA gibt es auch die direkte Funktion `IQR()`, die uns das ganze einfacher macht.
