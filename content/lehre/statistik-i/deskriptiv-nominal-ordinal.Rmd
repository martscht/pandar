...
```{r setup, cache = FALSE, echo = FALSE, warning = FALSE, message = FALSE, purl = FALSE}
...
Es treten die Werte `r min(fb22$prok4,na.rm=TRUE)` bis  `r max(fb22$prok4,na.rm=TRUE)` empirisch auf, außerdem gibt es  `r sum(is.na(fb22$prok4))` fehlende Werte (dargestellt als `NA`):
...
Der Interquartilsbereich liegt also zwischen `r quantile(fb22$prok4, .25, na.rm=TRUE)` und `r quantile(fb22$prok4, .75, na.rm=TRUE)`.
...
Dabei ist in der Ausgabe besonders die Überschrift verwirrend (75%), die hier nichts mit der Bedeutung des Wertes zu tun hat. Der IQA der Variable `prok4` beträgt `r quantile(fb22$prok4, .75, na.rm=TRUE) - quantile(fb22$prok4, .25, na.rm=TRUE)`. Für die Berechnung des IQA gibt es auch die direkte Funktion `IQR()`, die uns das ganze einfacher macht.
