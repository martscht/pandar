...
expected <- data.frame(
  nein = c(
    (tab_mar[1, 3] * tab_mar[3, 1]) / tab_mar[3, 3],
    (tab_mar[2, 3] * tab_mar[3, 1]) / tab_mar[3, 3]
  ),
  ja = c(
    (tab_mar[1, 3] * tab_mar[3, 2]) / tab_mar[3, 3],
...
chisq.test(tab, # Kreuztabelle
  correct = F
) # keine Kontinuinitätskorrektur nach Yates
