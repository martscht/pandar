...
* *$b_0$ (Achsenabschnitt)*: beträgt die Gewissenhaftigkeit 0, wird eine Vertrauen in die Psychologie als Wissenschaft von `r round(coef(lin_mod)[1],2)` vorhergesagt  
* *$b_1$ (Regressionsgewicht)*: mit jeder Steigerung der Gewissenhaftigkeit um 1 Einheit wird eine um `r abs(round(coef(lin_mod)[2],2))` Einheiten niedrigere (!) Vertrauen in die Psychologie als Wissenschaft vorhergesagt
...
`r round(r2, digits = 2)`% der Varianz von `trust` können durch `gewis` erklärt werden. Dieser Effekt ist nach Cohens (1988) Konvention als schwach bis mittel zu bewerten, wenn keine Erkenntnisse in dem spezifischen Bereich vorliegen.
