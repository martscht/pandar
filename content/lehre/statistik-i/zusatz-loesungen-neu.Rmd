...
Der Mittelwert von `M_ges` liegt bei `r round(mean(SD3$M_ges), digits = 3)`, der von `N_ges` bei `r round(mean(SD3$N_ges), digits = 3)`. Unter den getroffenen Annahmen sind die Proband:innen stärker machiavellistisch als narzisstisch. Die Streuung ist deskriptiv beim Narzissmus größer. Hier liegt sie bei `r round(var(SD3$N_ges) * (18190/18191), digits = 3)`, während sie beim Machiavellismus bei `r round(var(SD3$M_ges) * (18190/18191), digits = 3)` liegt.
...
```
Das Ergebnis des zweiseitigen Wilcoxon-Tests ist nicht signifikant (*W* = `r wilcox.test(nature$Q_ges ~ nature$urban_neu)$statistic `, *p* = `r round(wilcox.test(nature$Q_ges ~ nature$urban_neu)$p.value, 3)` ). Die Nullhypothese konnte nicht verworfen werden und wird beibehalten. Wir gehen also davon aus, dass sich Psychologiestudierende, die einen Nebenjob haben, und Psychologiestudierende, die keinen Nebenjob haben, nicht in ihrem Intellekt unterscheiden. 
