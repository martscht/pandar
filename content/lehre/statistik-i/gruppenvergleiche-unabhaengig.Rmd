...
Der Faktor hat also `r nlevels(fb23$fach_klin)` Stufen.
...
Anhand der Ergebnisse können wir folgende Aussage treffen: Studierende, die sich für klinische Inhalte interessieren unterscheiden sich in ihrer Verträglichkeit nicht bedeutsam von Studierenden, die sich primäre für andere Inhalte interessieren ($t = `r ttest$parameter`, df = `r round(ttest$statistic, 3)`$, $p = `r round(ttest$p.value,3)`$). 
...
Auch hier sehen wir ein nicht bedeutsames Ergebnis ($F_{`r levene2$Df[1]`, `r levene2$Df[2]`} = `r f2`$, $p = `r p2`$), sodass wir die Nullhypothese beibehalten und die Annahme der Homoskedastizität nicht verwerfen müssen.
...
Wir haben untersucht, ob klinisch interessierte Studierende im Mittel höhere Werte in der Lebenszufriedenheit erreichen als nicht klinisch interessierte Studierende. Deskriptiv besteht kaum ein Unterschied: Beide Gruppen weisen einen ähnlichen Median auf: $Md_\text{nicht klinisch}$ = `r round(deskr[['nicht klinisch']]$median,2)` und $Md_\text{klinisch}$ = `r round(deskr$klinisch$median, 2)`. Zur Überprüfung der Hypothese wurde ein Wilcoxon-Test durchgeführt. Der Gruppenunterschied ist nicht statistisch bedeutsam ($W = `r wilcox$statistic`$, $p = `r round(wilcox$p.value, 3)`$). Somit wird die Nullhypothese beibehalten: Klinisch interessierte Studierende erreichen keine höheren Werte in der Lebenszufriedenheit als nicht klinisch interessierte Studierende.
