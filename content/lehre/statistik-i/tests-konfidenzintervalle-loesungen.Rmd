...
```{r, echo = FALSE}
t_test1 <- t.test(fb23$extra, mu = 3.5, conf.level = 0.99)
conf_int1 <- t_test1$conf.int
```

Das 99%-ige Konfidenzintervall liegt zwischen `r round(conf_int1[1], digits = 2)` und `r round(conf_int1[2], digits = 2)`. Der Mittelwert der Gesamtbevölkerung ($\mu$ = 3.5) liegt außerhalb dieses Intervalls, somit kann mit einer Irrtumswahrscheinlichkeit von 1% die $H_0$ verworfen werden. Die Psychologie-Studierenden unterscheiden sich in ihrer Extraversion von der Gesamtbevölkerung. 
...
```{r, echo=FALSE}
t_test2 <- t.test(fb23$nerd, mu = 2.9, alternative = "greater")
p_test2 <- t_test2$p.value
```

Der p-Wert beträgt `r round(p_test2, digits = 4)` < .05, somit kann mit einer Irrtumswahrscheinlichkeit von 5% die $H_0$ verworfen werden. Die Psychologie-Studierenden haben höhere Nerdiness-Werte im Vergleich zur Gesamtbevölkerung.
...
Die Effektgröße ist mit `r round(d3, digits = 2)` nach Cohen (1988) als mittelstark einzuordnen.
