...
curve(expr = dnorm(x, mean = 2.5, sd = 3.1),
     ...
     ylab = "Dichte")
abline(v = mean(fb24$nerd, na.rm = TRUE),col=  "red")
```
...
```{r, echo = F, purl = F, fig.height = 4}
x <- seq(-0.5, 5.5, 0.1) 
y <- dnorm(x, 2.5, 3.1/sqrt(nrow(fb24)))
plot(x, y, type="l", 
      main = "SKV des MW für Nerdiness Population mit unserer Stichprobengröße", 
      xlab = "Nerdiness-Werte",
      ylab = "Dichte f(x)")
polygon(c(min(x), x[x<=mean(fb24$nerd, na.rm = TRUE)],  mean(fb24$nerd, na.rm = TRUE)), c(0, y[x<=mean(fb24$nerd, na.rm = TRUE)],  0), col="red")
abline(v = mean(fb24$nerd, na.rm = TRUE),col=  "red")
```
...
In diesem Fall würde man den wahren Mittelwert der Grundgesamtheit, aus der die Stichprobe gezogen wurde, zwischen `r round(lo_conf_nerd,2)` und `r round(up_conf_nerd,2)` vermuten. Mit einer Wahrscheinlichkeit von 95% enthält unser Konfidenzintervall zwischen `r round(lo_conf_nerd,2)` und `r round(up_conf_nerd,2)` den wahren Wert der Grundgesamtheit, aus der die Stichprobe stammt (also der Psychologiestudierenden des ersten Semesters).
...
```{r, echo = FALSE, purl = F}
neuro_test <- t.test(fb24$neuro, mu = 3.1, alternative = "greater", conf.level=0.99)
```
...
hist(fb24$neuro, xlim=c(0,6), main = "Histogramm",
     xlab = "Score", ylab= "Dichte", freq = FALSE)
curve(dnorm(x, 
            mean = mean(fb24$neuro, na.rm = TRUE), 
            sd = sd(fb24$neuro, na.rm = TRUE)), 
      add = T)
```
