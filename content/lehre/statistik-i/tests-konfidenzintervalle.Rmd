...
```{r, echo = F, purl = F, fig.height = 4}
curve(expr = dnorm(x, mean = 3.25, sd = 2.5),
     from = -1,
     to = 7.5,
     main = "Population", 
     xlab = "Vertrauen-Werte",
     ylab = "Dichte")
abline(v = 3.25,col=  "grey",lty = 2)
abline(v = mean(fb25$trust, na.rm = TRUE),col=  "red")
```
...
curve(expr = dnorm(x, mean = 3.25, sd = 2.5/sqrt(2)),
     from = -1,
     to = 7.5,
     main = "SKV des MW für Vertrauen Population mit n = 2", 
     xlab = "Vertrauen-Werte",
     ylab = "Dichte")
```
...
Der beobachtete Stichprobenmittelwert weicht demnach um $z_{emp}$ = `r round(z_emp,2)` *SE* (nach oben) vom Mittelwert der deutschen Wohnbevölkerung ab.  
...
```{r, echo = F, purl = F, fig.height = 4}
x <- seq(-1, 7.5, 0.1) 
y <- dnorm(x, 3.25, 2.5/sqrt(nrow(fb25)))
plot(x, y, type="l", 
      main = "SKV des MW für Vertrauen Population mit unserer Stichprobengröße", 
      xlab = "Vertrauen-Werte",
      ylab = "Dichte f(x)")
polygon(c(min(x), x[x<=mean(fb25$trust, na.rm = TRUE)],  mean(fb25$trust, na.rm = TRUE)), c(0, y[x<=mean(fb25$trust, na.rm = TRUE)],  0), col="red")
abline(v = mean(fb25$trust, na.rm = TRUE),col=  "red")
```
...
In diesem Fall würde man den wahren Mittelwert der Grundgesamtheit, aus der die Stichprobe gezogen wurde, zwischen `r round(lo_conf_trust,2)` und `r round(up_conf_trust,2)` vermuten. Mit einer Wahrscheinlichkeit von 95% enthält unser Konfidenzintervall zwischen `r round(lo_conf_trust,2)` und `r round(up_conf_trust,2)` den wahren Wert der Grundgesamtheit, aus der die Stichprobe stammt (also der Psychologiestudierenden des ersten Semesters).
...
Der *p*-Wert ist kleiner .01 ($p < \alpha$) (R gibt sehr kleine oder große Werte standardmäßig in wissenschaftlicher Notation mit Zehnerpotenzfaktor aus, voll ausgeschrieben lautet der Wert $p=`r format(pt(t_emp, df = sample_size - 1, lower.tail = F), scientific = FALSE)`$). Der Erwartungswert für den Mittelwert unserer Sichprobe $\mu_1$ ist demnach größer als der bekannte Populationsmittelwert $\mu_0$. Demnach würden wir die $H_0$ verwerfen und die $H_1$ annehmen.
...
```{r, echo = FALSE, purl = F}
neuro_test <- t.test(fb25$neuro, mu = 2.9, alternative = "greater", conf.level=0.99)
```
...
Die Effektgröße beträgt `r round(dt, 2)`, womit wir uns hier im Bereich eines kleinen Effektes befinden.
...
curve(dnorm(x, 
            mean = mean(fb25$neuro, na.rm = TRUE), 
            sd = sd(fb25$neuro, na.rm = TRUE)), 
      add = T)
