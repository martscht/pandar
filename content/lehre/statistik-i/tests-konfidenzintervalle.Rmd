...
Im Output sind bereits die wichtigsten Informationen enthalten. Wir erhalten den empirischen t-Wert $t_{emp}$ = `r round(neuro_test$statistic, 2)`. Der kritische Wert wird hier nicht ausgegeben, sondern stattdessen der p-Wert $p$ = `r round(neuro_test$p.value,6)`. Auch die Freiheitsgrade `df` = `r neuro_test$parameter` werden mit berichtet.
