...
```{r Erstellung eines Zufallsexperimentes}
wuerfel <- c(1:6)
...
hist(rowSums(expand.grid(wuerfel,wuerfel)),
        main = "Augensumme beim zweifachen Würfelwurf",
        xlab = "Augensumme",
        ylab = "Häufigkeit",
     breaks = c(1.5:12.5))
...
```{r, echo = F, fig.height = 4, purl = F}
plot(x = x, y = probs, type = "h", xlab = "Häufigkeiten des Ereignis Grün", ylab = "Wahrscheinlichkeit bei 100 Drehungen", col = c(rep("black",18),"red", rep("black",81)))
abline(h= 0.3, col = "red")
```
