...
Der Modus der Variable "wissen" beträgt `r which.max(table(fb23$wissen))`, d.h. diese Antwortkategorie wurde am häufigsten genannt (*n* = `r max(table(fb23$wissen))` bzw. `r round(max(prop.table(table(fb23$wissen)))*100,2)`%).
...
Der Median beträgt `r quantile(fb23$wissen, .5, na.rm=T)`. Das 1. und 3. Quartil betragen `r quantile(fb23$wissen, .25, na.rm=T)` bzw. `r quantile(fb23$wissen, .75, na.rm=T)`. Folglich sind die Grenzen des Interquartilsbereich (IQB) `r quantile(fb23$wissen, .25, na.rm=T)` und `r  quantile(fb23$wissen, .75, na.rm=T)`. Der Interquartilsabstand (IQA) beträgt `r  quantile(fb23$wissen, .75, na.rm=T)-quantile(fb23$wissen, .25, na.rm=T)`.
