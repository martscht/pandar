...
Extraversion lässt sich als einzige weitere Dimension der Big Five als signifikanter Prädiktor ($F = `r round(extra_anova[2, "F"], 2)`, p = `r round(extra_anova[2, "Pr(>F)"], 3)`$) für Prokrastinationstendenz in unser Modell aufnehmen. 
Dabei kann Extraversion zusätzlich `r round(100 * ink_extra, 2)`% Varianz erklären.
...
Zwei Personen die den **gleichen** Neurotizismus- sowie Extraversions-Wert haben, sich aber um eine Einheit in der Gewissenhaftigkeit unterscheiden, unterscheiden sich um $\mp `r round(abs(mod_final[["coefficients"]][["gewis"]]), 2)`$ Einheiten in der Prokrastinationstendenz.
...
Der Determinationskoeffizient ($R^2 =$ `r round(summary(mod_final)$r.squared, 4)`) besagt das `r round(100 * summary(mod_final)$r.squared, 2)`% der Varianz in der Prokrastinationstendenz durch unser Modell aus drei Prädiktoren (`neuro`, `gewis`, `extra`) erklärt wird.  
...
Das betragsmäßig größte standardisierte Regressionsgewicht hat Gewissenhaftigkeit mit `r round(mod_final_std[["standardized.coefficients"]][["gewis"]], 3)`.
