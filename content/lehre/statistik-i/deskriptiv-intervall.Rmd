...
Achtung! Wir benötigen für die Varianzberechnung `n` (s. Formel)! Wir nutzen hier `nrow(fb25)`-`r sum(is.na(fb25$lz))`, weil `nrow(fb25)` nicht das richtige n anzeigt (`r sum(is.na(fb25$lz))` Personen haben einen fehlenden Wert, daher die Anzahl an Zeilen minus der `r sum(is.na(fb25$lz))` fehlenden Werte = n).
...
Achtung! Dies funktioniert in unserem Fall wieder nicht, da die Verwendung von `nrow(fb25)` - wie oben bereits angemerkt - nicht sinnvoll ist: `nrow(fb25)` ist nicht gleich n (es kommt `NA` `r sum(is.na(fb25$lz))` Mal vor), daher besser:
