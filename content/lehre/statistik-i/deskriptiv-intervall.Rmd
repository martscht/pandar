...
Achtung! Wir benötigen für die Varianzberechnung `n` (s. Formel)! Wir nutzen hier `nrow(fb23)`-`r sum(is.na(fb23$lz))`, weil `nrow(fb23)` nicht das richtige n anzeigt (`r sum(is.na(fb23$lz))` Personen haben einen fehlenden Wert, daher die Anzahl an Zeilen minus der `r sum(is.na(fb23$lz))` fehlenden Werte = n).
...
Achtung! Dies funktioniert in unserem Fall wieder nicht, da die Verwendung von `nrow(fb23)` - wie oben bereits angemerkt - nicht sinnvoll ist: `nrow(fb23)` ist nicht gleich n (es kommt `NA` `r sum(is.na(fb23$lz))` Mal vor), daher besser:
