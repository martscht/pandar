...
```{r, echo=FALSE}
deskr <- describeBy(fb25$lz, fb25$ort)
fra <- summary(fb25[which(fb25$ort=="FFM"), "lz"])
and <- summary(fb25[which(fb25$ort=="anderer"), "lz"])
```
...
```{r, echo=FALSE}
wilcox <- wilcox.test(fb25$lz ~ fb25$ort, alternative = "greater",  conf.level = .95)
```

**Formales Berichten des Ergebnisses** 

Es wurde untersucht, ob in Frankfurt wohnende Studierende zufriedener im Leben sind als die außerhalb von Frankfurt wohnenden. Deskriptiv  zeigt sich, dass die Frankfurter:innen zufriedener sind (_Mdn_ = `r round(deskr$FFM$median, 1)`, _IQB_ = [`r round(fra[2], 1)` ; `r round(fra[5], 1)`]) als die Nicht-Frankfurter:innen (_Mdn_ = `r round(deskr$anderer$median, 1)`, _IQB_ = [`r round(and[2], 1)` ; `r round(and[5], 1)`]). Der entsprechende Wilcoxon-Test zeigt ein nicht signifikantes Ergebnis (_W_ = `r round(wilcox$statistic, 1)`, _p_ = `r round(wilcox$p.value, 3)`). Die Nullhypothese konnte nicht verworfen werden und wird beibehalten. Die Studierenden sind gleich zufrieden, unabhängig von ihrem Wohnort.
