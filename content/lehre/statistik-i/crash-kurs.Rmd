...
```{r Mögliche Argumentdarstellungen von log()}
log(100)
```
...
```{r Anwendungsordner auslesen, eval = FALSE}
getwd()
```
...
```{r Anwendungsordner neu setzen, eval = FALSE}
...
```{r Ordnerinhalte lesen, eval = FALSE}
dir()
```
...
```{r, echo = FALSE, purl = FALSE}
cat(paste(readLines('../../daten/fb25.csv')[1:5], collapse = '\n'))
```
...
```
```{r, echo = FALSE, error = TRUE, purl = FALSE}
fb25 <- read.table('../../daten/fb25.csv', header = TRUE)
```
...
```
```{r, echo = FALSE, purl = FALSE}
fb25 <- read.table('../../daten/fb25.csv', header = TRUE, sep = ",")
```
...
```{r, echo = FALSE, purl = FALSE}
cat(paste(readLines('fb25.txt')[1:5], collapse = '\n'))
```
