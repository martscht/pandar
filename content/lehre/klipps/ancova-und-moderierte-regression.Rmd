...
```{r, echo = F, message=F, warning = F, fig.height=6, fig.width=6, fig.height=6, fig.width=6}
library(plot3D)
# Übersichtlicher: Vorbereitung
x <- c(osf$pas_post)
y <- c(osf$bsi_post)
...
## R-Skript
Den gesamten `R`-Code, der in dieser Sitzung genutzt wird, können Sie [`r fontawesome::fa("download")` hier herunterladen](/post/KliPPs_MSc5a_R_Files/4_ancova-und-moderierte-regression_RCode.R).
...
cor(X_c, X_c*Z_c)
```
Die Korrelation zwischen $X$ und $X*Z$ liegt bei `r round(cor(X, X*Z), 3)`, während sie bei den zentrierten Varianten $X_c$ und $X_c*Z_c$ bei `r round(cor(X_c, X_c*Z_c), 3)` liegt. Damit resultiert die erste Variante in Multikollinearitätsproblemen in einem Regressionsmodell, während die zweite dies nicht tut.
