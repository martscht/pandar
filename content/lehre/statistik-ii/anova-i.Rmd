...
means <- conspiracy %>%
  group_by(urban) %>%
  summarise(Mittelwert = mean(EC))
gesamt_mean <- mean(conspiracy$EC)

# Select one person (e.g., ID = 30)
person <- conspiracy[14, ]
group_mean <- means$Mittelwert[means$urban == person$urban]

# Plot
ggplot(conspiracy, aes(urban, EC)) +
  geom_point() +
  geom_hline(yintercept = gesamt_mean, color = "black") +
  geom_hline(yintercept = group_mean, linetype = "dashed", color = "gray40") +
  geom_hline(yintercept = person$EC, linetype = "dotted", color = "gray40") +
  
  annotate("segment", x = 2.2, xend = 2.2, y = group_mean, yend = person$EC,
           arrow = arrow(length = unit(0.2, "cm"), ends = "both"), color = "red") +
  annotate("text", x = 2.3, y = (person$EC + group_mean)/2,
           label = "Fehlerwert", color = "red", hjust = 0) +
  
  annotate("segment", x = 2.3, xend = 2.3, y = group_mean, yend = gesamt_mean,
           arrow = arrow(length = unit(0.2, "cm"), ends = "both"), color = "red") +
  annotate("text", x = 2.4, y = (group_mean + gesamt_mean)/2,
           label = "Gruppenzugehörigkeit", color = "red", hjust = 0) +
  
  # annotate("segment", x = 2.1, xend = 2.1, y = 0, yend = gesamt_mean,
  #          arrow = arrow(length = unit(0.2, "cm"), ends = "both"), color = "red") +
  # annotate("text", x = 2.2, y = (gesamt_mean)/2,
  #          label = "Grundniveau", color = "red", hjust = 0) +
  
  annotate("segment", x = 2.0, xend = 2.0, y = gesamt_mean, yend = person$EC,
           arrow = arrow(length = unit(0.2, "cm"), ends = "both"), color = "red") +
  annotate("text", x = 1.9, y = (gesamt_mean + person$EC)/2,
           label = "Zu erklärender Anteil", color = "red", hjust = 1) +
  
  annotate("text", x = Inf, y = gesamt_mean, label = "Gesamtmittelwert",
           hjust = 1, vjust = -0.2, color = "black") +
  annotate("text", x = Inf, y = group_mean, label = "Gruppenmittelwert",
           hjust = 1, vjust = -0.2, color = "black") +
  annotate("text", x = Inf, y = person$EC, label = "Wert der Person",
           hjust = 1, vjust = -0.2, color = "black") + 
  labs(x = "Wohnraum", y = "EC-Wert") +
  ylim(2,5) +
