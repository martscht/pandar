...
Dieser reudzierte Datensatz besteht aus `r nrow(vegan)` Beobachtungen auf `r ncol(vegan)` Variablen. Für uns sind im Rahmen dieses Beitrags vor allem die `commitment` und die sechs Subskalen des VEMI+ relevant, die unterschiedliche Gründe für die vegane Ernährung abbilden. Mit einem Klick, finden Sie hier ein paar Beispielitems:
...
```
Inhaltlich ändert das wenig an den Aussagen - die konkreten Werte ändern sich zwar, aber bei $N = `r nrow(vegan)`$ ist die Datenlage zum Glück relativ robust.
