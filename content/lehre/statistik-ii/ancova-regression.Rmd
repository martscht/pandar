...
```{r Generalisierte-ANCOVA-Plot, fig = TRUE}
scatter + 
  # Kollektive Bestechung
...
```{r Komplexer-Plot(additiv), fig = TRUE}
scatter + 
...
```{r Komplexer-Plot-mit-Interaktion, fig = TRUE}
# Modell
...
```{r Komplexer-Plot-mit-vollständiger-Interaktion, fig = TRUE}
# Plot vorbereitung
tmp <- coef(mod13)
tab$intercept <- c(tmp[1], sum(tmp[c(1, 3)]), sum(tmp[1:2]), sum(tmp[c(1:3, 5)]))
