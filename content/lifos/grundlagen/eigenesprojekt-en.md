...
<details>
<summary>What can I do if I accidentally commited to a branch that is not <b>main</b>?</summary>

You can push your changes from the wrong branch to **main** and delete the accidental branch (if it was created on accident) in a few easy steps.

First, enter your project and click on the merge request symbol to the left.

Your changes on other branches should be visible as a merge request.

![](Gitlab_Project_Merge_Requests.png)

If you now click on *create merge request* a new window should open up

![](Gitlab_Project_New_Merge.png)

Here you now enter a title and description for your merge which should properly explain why and what you are merging from a different branch.

Afterwards, check the merge option `Delete source branch when merge request is accepted` if you intend to delete the branch onto which you accidentally commited your changes instead of **main** and press `Create merge request`.

You will now land on a new page.

![](Gitlab_Project_New_Merge_Confirm.png)

Now you need to confirm your merge and the squashing of the source branch from which you are merging once more by clicking the `Merge`-button.

After a short moment, you should now see that your merge suceeded!

![](Gitlab_Project_New_Merge_Sucess.png)

*If that is not the case, or you got stuck elsewhere in the process you can contact LIFOS[at]uni-frankfurt[punkt]de instead.*
</details>
