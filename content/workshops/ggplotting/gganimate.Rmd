...
```{r static-linedraw, fig = TRUE, echo = FALSE}
...
```{r faceted-scatter, fig = TRUE, fig.height = 9}
...
Wie wir schon für `ggplot2` gesehen haben, ist die Grundüberzeugung auch in der Umsetzung von `gganimate`, dass es leichter ist hier direkt Daten anzupassen, statt Veränderungen an der Darstellung vorzunehmen. Für unseren Fall heißt das, dass wir die Zeilen mit fehlenden Werten entfernen müssen. Dadurch werden die Übergänge zwischen den verbleibenden Jahren animiert, statt die Daten auszublenden. So würde für Indien dann der Übergang von 2003 zu 2009 als langsamer Verlauf zwischen (`r round(edu_exp[(edu_exp$geo == 'ind' & edu_exp$Year == 2003), 'Primary'], 2)`, `r round(edu_exp[(edu_exp$geo == 'ind' & edu_exp$Year == 2003), 'Index'], 2)`) und (`r round(edu_exp[(edu_exp$geo == 'ind' & edu_exp$Year == 2009), 'Primary'], 2)`, `r round(edu_exp[(edu_exp$geo == 'ind' & edu_exp$Year == 2009), 'Index'], 2)`) angezeigt werden.
...
```{r static-barchartrace, fig = TRUE, fig.height=9}
static + facet_wrap(~ Year, scales = 'free')
```
