...
```{r, eval = FALSE}
# Installation Für Einladen Excel Dateien 
...
Anmerkungen: Sollten Sie dieses Skript unabhängig vom Workshop lesen, gibt es die zugrundeliegenden Daten auch auf dem [OSF](https://osf.io/snqwr). Allerdings habe ich die für den Workshop ein paar Anpassungen vorgenommen, die hier [hier]() beschrieben werden. Am einfachsten wäre es, diesen Befehl auszuführen, der die Daten einliest und direkt in dem von mir durchgeführten Stil anpasst.
...
```{r}
class(data$Total_Competence_Maths)
```
...
Die Auswahl von Zeilen funktioniert mit der Funktion `filter()`. 

```{r}
data %>% slice(3)   # dplyr Funktion zur Auswahl der ersten Zeile
```
...
Als Beispiele nutzen wir die Berechnungen für verschiedene deskriptivstatistische Werte
...
```{r}
table(data$Total_Competence_Maths)
```
...
```{r}
min(data$Total_Competence_Maths)
```
...
Die Funktion `table()` ignoriert also einfach die fehlende Werte auf der Variable und gibt uns keinen Hinweis darauf. Außer wir würden die Gesamtsumme bilden gibt es hier keine Anzeichen für das Fehlen. Die Herangehensweise von Funktionen ist aber insgesamt nicht sehr konsistent. Betrachten wir beispielsweise den Mittelwert der Variable, passiert folgendes:
