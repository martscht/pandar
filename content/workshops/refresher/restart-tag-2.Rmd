...
Als Sie die demographischen Daten ihrer Stichprobe betrachten fällt ihnen auf das die Gruppe der Studierenden mit einem Anteil von `r round(stud_prop_table[2],4) * 100`% überrepräsentiert ist. Außerdem haben Sie erst letztens in einer Studie gelesen das Studierende eine unterdurchschnittlich geringen Verschwörungsglauben aufweisen.
...
Im Output sind bereits die wichtigsten Informationen enthalten. Wir erhalten den empirischen t-Wert $t_{emp}$ = `r round(one_sample_test[["statistic"]],2)`. Außerdem können wir auf Grund des ausgegebenen p-Werts erkennen das unser Ergebnis signifikant ist. Die Freiheitsgrade (`df`) werden auch berichtet.
...
Die Effektstärke bei unserer Hypothese beträgt $d$ = `r round(one_sample_eff[["estimate"]],2)`.
...
Anhand der Ergebnisse können wir folgende Aussage treffen: Ostdeutsche weisen eine signifkiant niedrigere Verschwörungsmentalität als Westdeutsche auf ($t$(`r indep_two_sample_test[["parameter"]]`) = `r round(indep_two_sample_test[["statistic"]],3)`, $p$ = `r round(indep_two_sample_test[["p.value"]],3)`).
...
Anhand der Ergebnisse können wir nun folgende Aussage treffen: Die Stimmung der Psychologiestudierenden vor und nach dem Besuch des ersten Statistik Praktika unterscheidet sich nicht signifikant. ($t$(`r dep_two_sample_test[["parameter"]]`) = `r round(dep_two_sample_test[["statistic"]],2)`, $p$ = `r round(dep_two_sample_test[["p.value"]],3)`).
